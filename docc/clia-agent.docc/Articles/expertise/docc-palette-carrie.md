...
@Metadata {
  @PageColor(blue)
  @PageImage(purpose: icon, source: "expertise-docc-palette-carrie-icon", alt: "expertise-docc-palette-carrie icon")
  @PageImage(purpose: card, source: "expertise-docc-palette-carrie-card", alt: "expertise-docc-palette-carrie card")
}

Generated CSS and JSON tokens for Carrie’s standardized DocC palette.
@Image(source: "expertise-docc-palette-carrie-hero", alt: "DocC palette - Carrie hero")
...
## Renderer Pipeline (Embedded Preview)

- The in‑process preview server (`docc-wrkstrm serve`) hosts a rendered DocC output directory.
- Use `docc-wrkstrm palette --format css --profile docc --palette carrie` to generate CSS tokens
  for catalog theming. Avoid runtime HTML injection; prefer static assets.
- When a custom renderer is required, vendor Swift‑DocC‑Render in your project and point DocC to it
  with `DOCC_HTML_DIR` during conversion/preview; keep license files intact.

## Verify

```bash
../clis/docc-wrkstrm-cli/.build/release/docc-wrkstrm palette \
  --format css \
  --profile docc \
  --palette carrie
